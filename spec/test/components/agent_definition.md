...
| `TestAgentDefinition_EmptyToolLists` | `unit` | Creates AgentDefinition with empty AllowedTools and DisallowedTools. | Temporary test directory created; `.spectra/agents/` directory created in test directory; `<test-dir>/.` directory exists; all file operations occur within test fixtures | `Role="SimpleAgent"`, `Model="sonnet"`, `Effort="medium"`, `SystemPrompt="Simple agent"`, `AgentRoot="."`, `AllowedTools=[]`, `DisallowedTools=[]` | Returns valid AgentDefinition; `AllowedTools=[]`, `DisallowedTools=[]` |
