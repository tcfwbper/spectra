# Test Specification: `workflowWatcher.test.ts`

## Source File Under Test
`vscode/src/services/workflowWatcher.ts`

## Test File
`vscode/test/suite/workflowWatcher.test.ts`
...
|---|---|---|---|---|---|
| `should store projectRoot` | `unit` | Construction stores the provided project root. | Stub `vscode.workspace.createFileSystemWatcher` to return a mock watcher with event stubs for `onDidCreate` and `onDidDelete`. Stub `vscode.RelativePattern` constructor. | `projectRoot='/project'` | Instance is created without error |
...
| `should create file system watcher with correct glob pattern` | `unit` | Construction creates a watcher targeting top-level yaml files in the workflows directory. | Spy on `vscode.workspace.createFileSystemWatcher`. Stub `vscode.RelativePattern` constructor to capture arguments. | `projectRoot='/my/root'` | `createFileSystemWatcher` called with a pattern matching `/my/root/.spectra/workflows/*.yaml` |

### Happy Path — onDidChange

| Test ID | Category | Description | Setup | Input | Expected |
|---|---|---|---|---|---|
| `should fire onDidChange after debounce when yaml file is created` | `unit` | A .yaml file creation results in a debounced event firing. | Stub watcher. Use fake timers. Register a spy on `instance.onDidChange`. | Trigger mock watcher's `onDidCreate` once | After advancing fake timer by 300ms, the `onDidChange` listener is called exactly once |
| `should fire onDidChange after debounce when yaml file is deleted` | `unit` | A .yaml file deletion results in a debounced event firing. | Stub watcher. Use fake timers. Register a spy on `instance.onDidChange`. | Trigger mock watcher's `onDidDelete` once | After advancing fake timer by 300ms, the `onDidChange` listener is called exactly once |
| `should debounce rapid successive create and delete signals into single event` | `unit` | Multiple rapid create/delete signals within the debounce window produce only one event. | Stub watcher. Use fake timers. Register a spy on `instance.onDidChange`. | Trigger `onDidCreate` twice then `onDidDelete` once with 100ms between each | After advancing fake timer to 300ms past the last trigger, the `onDidChange` listener is called exactly once total |
| `should reset debounce timer on each new signal` | `unit` | Each new file-change signal resets the 300ms quiet period. | Stub watcher. Use fake timers. Register a spy on `instance.onDidChange`. | Trigger `onDidCreate`, advance 200ms, trigger `onDidDelete`, advance 200ms, trigger `onDidCreate` | After advancing 300ms from the last trigger, listener called once; at 200ms from last trigger, listener not yet called |

### Happy Path — Ignoring Modifications

| Test ID | Category | Description | Setup | Input | Expected |
|---|---|---|---|---|---|
| `should not fire onDidChange when yaml file content is modified` | `unit` | File content modifications do not trigger any event. | Stub watcher with `onDidChange` event hook (the watcher's internal change event). Use fake timers. Register a spy on `instance.onDidChange`. | Trigger mock watcher's `onDidChange` (file modification), advance timer by 500ms | The `instance.onDidChange` listener is never called |
...
| Test ID | Category | Description | Setup | Input | Expected |
|---|---|---|---|---|---|
| `should subscribe only to onDidCreate and onDidDelete` | `unit` | Only file creation and deletion events are subscribed, not modification. | Stub `vscode.workspace.createFileSystemWatcher` to return a mock watcher with spies on `onDidCreate`, `onDidChange`, `onDidDelete`. | `projectRoot='/project'` | `onDidCreate` and `onDidDelete` subscriptions are registered; `onDidChange` is not subscribed |
