...
| `TestWorkflowDefinitionLoader_Load_YamlContainsNameField` | `unit` | Rejects YAML containing a `name` field as an unknown field since Name is derived exclusively from filename. | Create temp dir with `.spectra/workflows/Legacy.yaml` containing valid workflow structure plus a top-level `name: Legacy` field. Inject mock AgentLoader. | `workflowName="Legacy"` | Returns error matching `"failed to parse workflow definition 'Legacy':"` |
