...
| `TestSessionInitializer_Initialize_PassesOsPidToNewSession` | `unit` | Passes `os.Getpid()` as the pid argument to NewSession. | Mock all dependencies to succeed. Inject or observe the NewSession call arguments (e.g., via a constructor seam or by verifying the resulting session's Pid field). | `si.Initialize("wf", "", make(chan struct{}, 2))` | The constructed Session has `Pid == os.Getpid()` (the current process PID) |
