...
describe("SessionTerminator", function () {
  ...
  describe("Happy Path — terminate", function () {
    it("should return terminated with sigterm when process responds to SIGTERM", async function () {
    ...
    it("should return terminated with sigkill when process ignores SIGTERM", async function () {
    ...
    it("should log info when SIGTERM is sent", async function () {
    ...
    it("should log warn when escalating to SIGKILL", async function () {
      deps.processKill.withArgs(100, 0).returns(undefined);
      makePsReturn(deps.execFile, "spectra");
      deps.processKill.withArgs(100, "SIGTERM").returns(undefined);
  ...
  describe("Happy Path — already dead", function () {
    it("should return already_dead when process does not exist", async function () {
    ...
    it("should return already_dead when process dies between check and SIGTERM", async function () {
  ...
  describe("Boundary Values — command name matching", function () {
    it("should match when ps reports basename of configured path", async function () {
    ...
    it("should match when ps reports literal spectra regardless of config", async function () {
    ...
    it("should match custom binary name from config basename", async function () {
    ...
    it("should return not_spectra when command name does not match", async function () {
      deps.processKill.withArgs(100, 0).returns(undefined);
  ...
  describe("Error Propagation", function () {
    it("should return error result when SIGTERM fails with EPERM", async function () {
      deps.processKill.withArgs(100, 0).returns(undefined);
      makePsReturn(deps.execFile, "spectra");
    ...
    it("should return not_spectra with error when ps command fails", async function () {
      deps.processKill.withArgs(100, 0).returns(undefined);
    ...
    it("should never throw to caller", async function () {
  ...
  describe("Mock / Dependency Interaction", function () {
    it("should poll liveness every 500ms during grace period", async function () {
      deps.processKill.withArgs(100, 0).returns(undefined);
      makePsReturn(deps.execFile, "spectra");
      deps.processKill.withArgs(100, "SIGTERM").returns(undefined);
      ...
      const livenessCalls = deps.processKill.getCalls()
        .filter((c: any) => c.args[1] === 0);
      expect(livenessCalls.length).to.be.at.least(5);
    });

    it("should use 5 second grace period before SIGKILL", async function () {
      deps.processKill.withArgs(100, 0).returns(undefined);
      makePsReturn(deps.execFile, "spectra");
      deps.processKill.withArgs(100, "SIGTERM").returns(undefined);
    ...
    it("should wait 500ms after SIGKILL to confirm death", async function () {
      deps.processKill.withArgs(100, 0).returns(undefined);
      makePsReturn(deps.execFile, "spectra");
      deps.processKill.withArgs(100, "SIGTERM").returns(undefined);
    ...
    it("should not send any signal when process is not_spectra", async function () {
      deps.processKill.withArgs(100, 0).returns(undefined);
      ...
      const signalCalls = deps.processKill.getCalls()
        .filter((c: any) => c.args[1] === "SIGTERM" || c.args[1] === "SIGKILL");
