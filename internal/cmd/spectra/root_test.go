...
func TestRoot_RegistersInitSubcommand(t *testing.T) {
	t.Skip("scaffolded: requires root.go to define NewRootCommand() returning *cobra.Command")

	// Verify root command's subcommands include "init".
}

func TestRoot_RegistersRunSubcommand(t *testing.T) {
	t.Skip("scaffolded: requires root.go to define NewRootCommand() returning *cobra.Command")

	// Verify root command's subcommands include "run".
}

func TestRoot_RegistersClearSubcommand(t *testing.T) {
	t.Skip("scaffolded: requires root.go to define NewRootCommand() returning *cobra.Command")

	// Verify root command's subcommands include "clear".
}
...
func TestRoot_PropagatesSubcommandExitCode0(t *testing.T) {
	t.Skip("scaffolded: requires root.go to expose subcommand registration seam for stub commands")
