...
func TestMain_ExitCode2(t *testing.T) {
	exitRec, _ := installFakes(t, 2)

	main()

	assert.Equal(t, 1, exitRec.callCount(), "osExit should be called exactly once")
	assert.Equal(t, 2, exitRec.lastCode(), "exit code should be 2")
}

func TestMain_ExitCode3(t *testing.T) {
	exitRec, _ := installFakes(t, 3)

	main()

	assert.Equal(t, 1, exitRec.callCount(), "osExit should be called exactly once")
	assert.Equal(t, 3, exitRec.lastCode(), "exit code should be 3")
}
...
func TestMain_NoOtherOsCalls(t *testing.T) {
	...
	require.NoError(t, err, "failed to parse cmd/spectra_agent/main.go")
